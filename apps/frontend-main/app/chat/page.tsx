  ...
  return (
    <div className={`flex ${user === "AI" ? "justify-start" : "justify-end"}`}>
      <div className="max-w-sm md:max-w-md lg:max-w-lg p-3 px-5 rounded-lg shadow-md bg-[#262626] text-[#f2f2f2]">
        ...
        {timestamp && <div className="text-xs text-gray-400 mt-1">{new Date(timestamp).toLocaleTimeString()}</div>}
      </div>
    </div>
  );
};
  ...
  const callLangflow = async (userInput: string) => {
    try {
      ...
      setMessages((prevMessages) => [
        ...
        { user: "AI", text: botResponse?.text || "No response", timestamp: botResponse?.timestamp },
      ]);
    } catch (error) {
      console.error("Error calling Langflow API:", error);
      setMessages((prevMessages) => [...prevMessages, { user: "AI", text: "Error fetching response." }]);
    }
  };
     ...
     <PageContainer >
      <Card className="flex flex-col h-screen w-full max-w-3xl mx-auto border rounded-lg shadow-lg bg-[#121212]">
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">Ask anything!</CardDescription>
        </CardHeader>
        ...
        <div className="flex items-center gap-3 p-4 border-t">
          ...
          />
          <Button onClick={sendMessage} className="px-4 py-2 text-white rounded-lg">Send</Button>
        </div>
      </Card>
      </PageContainer>
    </ChatLayout>
